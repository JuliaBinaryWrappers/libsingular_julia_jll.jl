...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libsingular_julia_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"libsingular_julia"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("libsingular_julia_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "libsingular_julia_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"libsingular_julia", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "libsingular_julia_jll", "src", "libsingular_julia_jll.jl"))
        @info("libsingular_julia_ll dev'ed out to /depot/dev/libsingular_julia_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("ae4fbd8f-ecdb-54f8-bbce-35570499b30e"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libsingular_julia", artifacts_toml) for e in artifacts["libsingular_julia"]]
...
if best_platform === nothing
    @debug("Unable to load libsingular_julia; unsupported platform $(triplet(platform_key_abi()))")
else
